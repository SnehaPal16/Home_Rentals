...
const CreateListing = () => {
    ...
    const handlePost = async (e) => {
        ...
        try {
            ...
            const response = await axios.post("http://localhost:8080/properties/create", listingForm);
    
            if (response.status === 201) {
                toast.success("Listing created successfully!");
                navigate("/");
            } else {
                toast.error(response.data.message || "Failed to create the listing.");
            }
        } catch (err) {
            toast.error("Public listing failed: " + err.message);
        }
    };
                    ...
                    <div className='type-list'>
                        {types?.map((item , index) => (
                            <div className={`type ${type === item.name ? "selected" : ""}`} key={index} onClick={() => setType(item.name)}>
                                ...
                                <div className='types_icon'>{item.icon}</div>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <Droppable droppableId='photos' direction='horizontal'>
                        {(provided) => (
                            <div className='photos' {...provided.droppableProps} ref={provided.innerRef}>
                                {photos.length < 1 && (
                                    <div>
                                        <input id='image' type='file' style={{display : "none"}} accept='image/*' onChange={handleUploadPhotos}/>
